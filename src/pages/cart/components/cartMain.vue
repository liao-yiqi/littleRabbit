...
import { useGoTop, useGuessList } from '@/composables'
import {
  getMemberCartAPI,
  deleteMemberCartAPI,
...
const getMemberCartData = async () => {
  ...
  showCartList.value = result.length > 0
}
...
const isSelectedAll = computed(() => {
  return memberCartList.value.length && memberCartList.value.every((item) => item.selected)
})

const onSelectedAll = async () => {
  const _isSleectedAll = !isSelectedAll.value
  memberCartList.value.forEach((item) => (item.selected = _isSleectedAll))
  await putMemberCartSelectedAPI({ selected: _isSleectedAll })
}
...
<template>
  ...
  >
    ...
    <template v-if="memberStore.profile">
      ...
      >
        <text class="all" :class="{ checked: isSelectedAll }" @tap="onSelectedAll">全选</text>
        <text class="text">合计</text>
        <text class="amount">{{ selectedCartListMoney }}</text>
        <view class="button-grounp" @tap="onGoPayment">
          <view class="button payment-button" :class="{ disabled: !selectedCartList.length }">
